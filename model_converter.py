...
This module converts weights from the original Stable Diffusion checkpoint format
to match our custom architecture. The original implementation uses different naming
conventions and sometimes slightly different layer structures.

The conversion maps thousands of weight tensors from formats like:
- "model.diffusion_model.input_blocks.0.0.weight"
to our format like:
- "diffusion.unet.encoders.0.0.weight"

This is a large but necessary mapping to use pre-trained weights with our implementation.
The mapping was determined by analyzing the original Stable Diffusion codebase.
"""
...
def load_from_standard_weights(input_file: str, device: str) -> dict[str, torch.Tensor]:
    """
    Load and convert weights from original Stable Diffusion checkpoint.
    
    Args:
        input_file: Path to checkpoint file (.ckpt or .safetensors)
        device: Device to load weights on
    
    Returns:
        Dictionary with converted weights organized by model component:
        - 'encoder': VAE encoder weights
        - 'decoder': VAE decoder weights
        - 'diffusion': UNET diffusion model weights
